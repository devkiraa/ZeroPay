    ...
    <div>
      ...
      <div className="bg-white rounded-lg shadow border border-gray-200 overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {merchants.length > 0 ? (
              merchants.map((merchant: { _id: unknown; name: string; email: string; publicKey: string; createdAt: string }) => (
      ...
      <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
        <p className="text-sm text-blue-800">
          <strong>Note:</strong> Secret keys are hidden for security. Merchants can regenerate their own keys from their dashboard.
        </p>
      </div>
    </div>
  );
