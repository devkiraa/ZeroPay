...
async function getAllTransactions() {
  ...
  const merchantIds = [...new Set(transactions.map(tx => String(tx.merchantId)))];
  const merchants = await Merchant.find({ _id: { $in: merchantIds } }).select('name email');
  
  const merchantMap = new Map(merchants.map(m => [String(m._id), { name: m.name, email: m.email }]));
