...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!["card", "upi", "wallet", "netbanking"].includes(method)) {
      return NextResponse.json(
    ...
    // 6. Simulate payment processing (mock real gateway)
    // For UPI, simulate a delay and then mark as success
    await new Promise((resolve) => setTimeout(resolve, 2000)); // 2s loader
    savedTransaction.status = "success";
    await savedTransaction.save();

    // 7. Return the details to the merchant's backend
    return NextResponse.json(
      {
        success: true,
